...
function Calc(data) {
	var key = base32tohex(data.secret);
...
Page({
  ...
  onLoad:function(options){
	...
	if (wx.getStorageInfoSync().keys.indexOf('credentials') < 0)
		wx.setStorageSync('credentials', [])
  },
  onReady:function(){
    // 页面渲染完成
  },
  ...
  refreshOTP:function() {
	if (wx.getStorageInfoSync().keys.indexOf('credentials') < 0)
		wx.setStorageSync('credentials', [])
	var storage = wx.getStorageSync('credentials')
	if (storage.length == 0)
		this.setData({
			show_perc : "none"
		})
    ...
    this.setData({
      codes : storage.map(Calc)
    })
  },
  timer:function() {
    var epoch = Math.round(new Date().getTime() / 1000.0);
    var countDown = 30 - (epoch % 30);
    if (epoch % 30 == 0) this.refreshOTP();
    this.setData({
      perc : countDown / 30 * 100
    })
  },
  resetAction:function(e) {
	if (!this.data.is_longpress)
		this.setData({
			show_scan : "normal",
			show_delete : "none",
			to_delete : ""
  ...
  codeAction:function(e) {
	this.setData({
		show_scan : "none",
		show_delete : "normal",
		to_delete : e.currentTarget.id,
		is_longpress : true
	})
  },
  deleteAction:function() {
	this.delOTP(this.data.to_delete)
	this.refreshOTP()
  },
  syncAction:function() {
  },
  scanAction:function() {
	wx.scanCode({
	  success: (res) => {
		this.addOTP(res.result)
	  }
	})
  },
  addOTP:function(uri) {
	var storage = wx.getStorageSync('credentials')
	if (ParseCode(uri) != false)
		storage.push(ParseCode(uri))
	wx.setStorageSync('credentials', storage)
	this.refreshOTP()
  },
  delOTP:function(id) {
	wx.setStorageSync('credentials', 
		wx.getStorageSync('credentials').filter(
			function(item) {
				return item.guid != id
			}
		)
